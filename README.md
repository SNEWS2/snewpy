...
### Earth-Matter Effect
Navigate to python/snewpy/SQA and run: `python setup.py install' preferably as root. All the ususal setup options are available if you want a non-standard install. If you don't run as root you need to set the PYTHONPATH environment variable, or have a copy of the module in your working directory. You may need to modify the setup file depenidng upon your particular machine. A version of setup that worked on a Mac's is provided. 

Whenever you use the module you need to provide a density and an electron fraction profile of the Earth. The PREM is provided. The script SNEWS2.0_rate_table_singleexample+EME.py found in python/snewpy/scripts uses the module. 
