...
from snewpy.models.base import PinchedModelBase, SupernovaModel
...
class _BasicLoader:
    @classmethod
    def get_file(cls, file_path:str) -> Path:
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f'File requested from "{__name__}.{cls.__name__}" not found: {file_path}')
        return path


class PinchedModel(PinchedModelBase, _BasicLoader):
    def __init__(self, filename, metadata={}):
        ...
        datafile = self.get_file(filename)
...
class GarchingArchiveModel(PinchedModelBase, _BasicLoader):
    ...
    def __init__(self, filename, eos='LS220', metadata={}):
        ...
        for flavor in Flavor:
            ...
            # Open the requested filename using the model downloader.
            datafile = datafile = self.get_file(_filename)
...
class OConnor_2013(PinchedModelBase, _BasicLoader):
...
class OConnor_2015(PinchedModelBase, _BasicLoader):
...
class Warren_2020(PinchedModelBase, _BasicLoader):
    def __init__(self, filename, metadata={}):
        """
        Parameters
        ----------
...
class Kuroda_2020(PinchedModelBase, _BasicLoader):
    def __init__(self, filename, metadata={}):
        """
        Parameters
        ----------
...
class Fornax_2019(SupernovaModel, _BasicLoader):
