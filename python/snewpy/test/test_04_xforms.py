...
class TestFlavorTransformations(unittest.TestCase):
    ...
    def test_quantumdecoherence_nmo(self):
        ...
        """
        # Override the default mixing angles.
        xform = QuantumDecoherence(mix_angles=(self.theta12, self.theta13, self.theta23), Gamma3=self.gamma3, Gamma8=self.gamma8, dist=self.distance, n=self.n, E0=self.energy_ref, mh=MassHierarchy.NORMAL)
        ...
        self.assertTrue(xform.P33(_E) == 1/3 + 2/3 * np.exp(-self.gamma8 * (_E/self.E0)**self.n * self.distance))
        ...
        self.assertTrue(np.array_equal(xform.prob_ee(self.t, self.E), prob_ee))
        self.assertEqual(xform.prob_ex(self.t, self.E), 1 - prob_ee)
        self.assertEqual(xform.prob_xx(self.t, self.E), 1 - 0.5*(1 - prob_ee))
        self.assertTrue(np.array_equal(xform.prob_xe(self.t, self.E), 0.5*(1 - prob_ee)))

        prob_eebar = np.asarray([xform.P11(_E)*De1 + xform.P21(_E)*De2 + xform.P31(_E)*De3 for _E in self.E])

        self.assertEqual(xform.prob_exbar(self.t, self.E), 1 - prob_eebar)
        self.assertTrue(np.array_equal(xform.prob_xxbar(self.t, self.E), 1. - 0.5*(1 - prob_eebar)))
        self.assertEqual(xform.prob_xebar(self.t, self.E), 0.5*(1. - prob_eebar))
    ...
    def test_quantumdecoherence_nmo_default_mixing(self):
        ...
        """
        # Use default mixing angles defined in the submodule.
        xform = QuantumDecoherence(Gamma3=self.gamma3, Gamma8=self.gamma8, dist=self.distance, n=self.n, E0=self.energy_ref)
    ...
    def test_quantumdecoherence_imo(self):
        ...
        """
        # Override the default mixing angles.
        xform = QuantumDecoherence(mix_angles=(self.theta12, self.theta13, self.theta23), Gamma3=self.gamma3, Gamma8=self.gamma8, dist=self.distance, n=self.n, E0=self.energy_ref, mh=MassHierarchy.INVERTED)
    ...
    def test_quantumdecoherence_imo_default_mixing(self):
        ...
        """
        # Use default mixing angles defined in the submodule.
        xform = QuantumDecoherence(Gamma3=self.gamma3, Gamma8=self.gamma8, dist=self.distance, n=self.n, E0=self.energy_ref, mh=MassHierarchy.INVERTED)
