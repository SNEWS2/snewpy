...
transformations = [ Catenate(AdiabaticMSW_NMO,VacuumTransition,EarthMatter_NMO), \
                    Catenate(AdiabaticMSW_IMO,VacuumTransition,EarthMatter_IMO) ]
...
if (have_data_saved is False):
    ...
    for model in models:
        ...
        for transformation in transformations:
            total_events[model][str(transformation)] = {}

            file_name = models[model]['file_name']
            modeltype = models[model]['type']
            outfile = modeltype+"_"+model+"_summed_"+str(transformation)
            ...
            for det in dets:
                ...
                total_events[model][str(transformation)][det+"smeared"]=0
                for j in range(1,len(tables[key]['header'].split())):
                    total_events[model][str(transformation)][det+"smeared"] += sum(tables[key]['data'][j])

                key = "Collated_"+outfile+"_"+det+"_events_unsmeared_weighted.dat"
                total_events[model][str(transformation)][det+"unsmeared"]=0
                for j in range(1,len(tables[key]['header'].split())):
                    total_events[model][str(transformation)][det+"unsmeared"] += sum(tables[key]['data'][j])
