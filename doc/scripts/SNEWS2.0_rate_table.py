...
for effects in detector_effects:
    ...
    for model in models:
        ...
        for transform in transformations:
            total_events[effects][model][transform] = {}
            #get the flux from the model
            flux = models[model].get_flux(t=times, E=energies, distance=distance, flavor_xform=transformations[transform])
            fluence = flux.integrate('time')
            for detector in detectors:
                events = rc.run(fluence, detector, detector_effects=detector_effects[effects])
                total_events[effects][model][transform][detector] = sum([chan.integrate_or_sum('energy').array.squeeze().value for chan in events.values()]) 
