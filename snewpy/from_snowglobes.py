...
def collate(Branch, Model_Path, Tarball, file_name, detector_input = all, skip_plots=True, return_tables=True):
    ...
    for folder in os.listdir(Branch + "out"):
        if os.path.isdir(Branch + "out/" + folder):
            for file in os.listdir(Branch + "out/" + str(folder)):
                if str(file)[0:5] != "Final" and str(file) != "plots":
                    os.remove(Branch + "out/" + str(folder) + "/" + str(file))
                else:
                    continue
        else:
            for file in os.listdir(Branch + "out"):
                os.remove(Branch + "out/" + str(file))
